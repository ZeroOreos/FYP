...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("gallery_dir", type=Path, help="Path to the clean gallery dataset directory.")
...
def build_main_attack_cmd(main_script: Path, gallery_dir: Path, args: argparse.Namespace) -> list[str]:
    ...
    if args.probe_dir:
        cmd.extend(["--probe-dir", str(args.probe_dir)])
    if args.attack_method:
        cmd.extend(["--attack-method", args.attack_method])
    if args.attack_generator_script:
        cmd.extend(["--attack-generator-script", str(args.attack_generator_script)])
...
def main() -> int:
    ...
    print_run_header("slowed/main_attack_slowed.py", "Gallery directory:", gallery_dir, main_script, args.python_bin)
