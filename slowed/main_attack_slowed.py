#!/usr/bin/env python3
# python3 main_attack_slowed.py <gallery_dir> [throttle options] -> throttled attack pipeline
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run main_attack.py with thermal-aware control: plugin throttling or process-level pause.",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to main_attack.py. Defaults to main_attack.py in the project root.",
    )
    parser.add_argument(
        "--python-bin",
        type=str,
    ...
    throttle_group.add_argument(
        "--plugin-throttle",
        action="store_true",
        help="Use in-process throttling: pass --throttle through to main_attack.py (recommended).",
    )
    throttle_group.add_argument(
        "--pause-generate",
        action="store_true",
        help="Only SIGSTOP during model generate.py stages, skip attack-pair / evaluate / compile stages.",
    )

    parser.add_argument("--probe-dir", type=str, default=None)
    parser.add_argument("--attack-method", type=str, default=None)
    parser.add_argument("--attack-generator-script", type=str, default=None)
    parser.add_argument("--pair-model", type=str, default="InsightFace")
    ...
    parser.add_argument("--run-seconds", type=int, default=480)
    ...
    parser.add_argument("--cpu-check-window", type=int, default=5)
    parser.add_argument("--extra-cooldown-seconds", type=int, default=90)
    parser.add_argument("--disable-load-trigger", action="store_true")
    parser.add_argument("--pause-all-stages", action="store_true")
    parser.add_argument("--disable-main-throttle", action="store_true")
    parser.add_argument("--min-cooldown-seconds", type=int, default=20)
    parser.add_argument("--max-cooldown-seconds", type=int, default=240)
    parser.add_argument("--inter-section-cooldown-seconds", type=int, default=90)
    parser.add_argument("--disable-section-cooldown", action="store_true")
...
def resolve_main_script(path_arg: str | None) -> Path:
    if path_arg:
        ...
        main_path = (Path(__file__).resolve().parent.parent / "main_attack.py").resolve()
    if not main_path.exists():
        raise FileNotFoundError(f"main_attack.py not found at: {main_path}")
    return main_path
...
def build_main_attack_cmd(main_script: Path, gallery_dir: Path, args: argparse.Namespace, include_throttle: bool) -> list[str]:
...
def main() -> int:
    ...
    print("[INFO] === slowed/main_attack_slowed.py thermal-aware orchestrator ===")
    ...
    if args.plugin_throttle:
        return run_with_plugin_throttle(main_script, gallery_dir, args)
    return run_with_pause_generate(main_script, gallery_dir, args)


def run_with_plugin_throttle(main_script: Path, gallery_dir: Path, args: argparse.Namespace) -> int:
    cmd = build_main_attack_cmd(main_script, gallery_dir, args, include_throttle=True)
    print("\n[MODE] Plugin-level throttling (RECOMMENDED)")
    print("[INFO] Passing --throttle to main_attack.py")
    ...
    try:
        ...
        print(f"[ERROR] Failed to run main_attack.py: {exc}", file=sys.stderr)
        return 1


def run_with_pause_generate(main_script: Path, gallery_dir: Path, args: argparse.Namespace) -> int:
    cmd = build_main_attack_cmd(main_script, gallery_dir, args, include_throttle=not args.disable_main_throttle)
    ...
    try:
        ...
        print(f"[ERROR] Failed to start main_attack.py: {exc}", file=sys.stderr)
        return 1
    ...
    print(f"[INFO] Started main_attack.py with PID {popen.pid}")
    try:
    ...
    try:
        while True:
            ret = popen.poll()
            if ret is not None:
                print(f"\n[INFO] main_attack.py exited with code {ret}")
                return ret
            ...
            if generate_active and generate_session_key != current_generate_session_key:
                ...
                if seen_generate_sessions > 1 and not args.disable_section_cooldown and args.inter_section_cooldown_seconds > 0:
            ...
            if fixed_cycle_due or load_cycle_due:
                reason = "fixed pacing" if fixed_cycle_due and not load_cycle_due else "high CPU load" if load_cycle_due and not fixed_cycle_due else "fixed pacing + high CPU load"
                cooldown_time = adaptive_cooldown_seconds(args, avg_cpu, load_cycle_due)
                print(f"\n[INFO] Pausing main_attack.py due to {reason}.")
                print(f"[INFO] Adaptive cooldown: {cooldown_time}s")
                stop_process_tree(proc)
                was_paused = True
                cycle_count += 1
                sleep_with_progress(cooldown_time, "[COOLDOWN]")
                print("[INFO] Resuming main_attack.py.")
                continue_process_tree(proc)
                was_paused = False
                run_start = time.time()
                cpu_samples.clear()
