...
DEFAULTS = external_backend_defaults(
    "mordiff",
    "MorDIFF_upstream",
    default_entry_script="morph_two_images.py",
)
...
def parse_args() -> argparse.Namespace:
    ...
    return resolve_external_backend_args(
        parser.parse_args(),
        defaults=DEFAULTS,
        repo_env="FYP_MORDIFF_REPO_DIR",
        entry_env="FYP_MORDIFF_ENTRY_SCRIPT",
        checkpoint_env="FYP_MORDIFF_CHECKPOINT",
    )
...
def build_command(args: argparse.Namespace, source_path: Path, target_path: Path, pair_dir: Path) -> tuple[list[str], Path]:
    if args.entry_script is None:
        raise ValueError("MorDIFF wrapper could not find an entry script. Put the backend source in Backends/sources/ or pass --entry-script.")
...
def main() -> None:
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            run_command(cmd, cwd=args.repo_dir, stage_name="mordiff")
            generated = resolve_output_file(result_path, pair_dir)
            if generated is None:
                raise RuntimeError("MorDIFF backend completed but no output image was found.")
