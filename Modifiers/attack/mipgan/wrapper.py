...
DEFAULTS = external_backend_defaults(
    "mipgan",
    "MIPGAN_upstream",
    default_entry_script="morph.py",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate MIPGAN attack probes with an external backend wrapper.")
    ...
    parser.add_argument("--source-arg", type=str, default="--source")
    parser.add_argument("--target-arg", type=str, default="--target")
    parser.add_argument("--output-arg", type=str, default="--output")
    parser.add_argument("--checkpoint-arg", type=str, default="--checkpoint")
    parser.add_argument("--extra-arg", action="append", default=[])
    parser.add_argument("--overwrite", action="store_true")
    return resolve_external_backend_args(
        parser.parse_args(),
        defaults=DEFAULTS,
        repo_env="FYP_MIPGAN_REPO_DIR",
        entry_env="FYP_MIPGAN_ENTRY_SCRIPT",
        checkpoint_env="FYP_MIPGAN_CHECKPOINT",
    )
...
def build_command(args: argparse.Namespace, source_path: Path, target_path: Path, pair_dir: Path) -> tuple[list[str], Path]:
    if args.entry_script is None:
        raise ValueError("MIPGAN wrapper could not find an entry script. Put the backend source in Backends/sources/ or pass --entry-script.")

    result_path = pair_dir / "result.png"
    cmd = [args.python_bin, str(args.entry_script), args.source_arg, str(source_path), args.target_arg, str(target_path), args.output_arg, str(result_path)]
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            if generated is None:
                raise RuntimeError("MIPGAN backend completed but no output image was found.")
