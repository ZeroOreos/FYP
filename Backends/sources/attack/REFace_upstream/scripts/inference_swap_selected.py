...
# load safety model lazily; the selected-image inference path does not need it
...
def main():
    ...
    for i,im in enumerate(os.listdir(source_cropped_path)):
        ...
        ref_dtype = torch.float16 if device.type == "cuda" else torch.float32
        ref_image_tensor = ref_img.to(device, non_blocking=True).to(ref_dtype).unsqueeze(0)
        ...
        use_autocast = opt.precision == "autocast" and device.type == "cuda"
        precision_scope = autocast if use_autocast else nullcontext
        sample=0
        with torch.no_grad():
            with precision_scope("cuda" if use_autocast else None):
