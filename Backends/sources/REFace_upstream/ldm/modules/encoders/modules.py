...
def _resolve_hf_version(version: str) -> str:
    local_dir = os.environ.get("REFACE_CLIP_MODEL_DIR")
...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14"):
        super().__init__()
        version = _resolve_hf_version(version)
        self.transformer = CLIPVisionModel.from_pretrained(version, local_files_only=os.path.isdir(version))
...
class FrozenCLIPTextEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14"):
        super().__init__()
        version = _resolve_hf_version(version)
        local_only = os.path.isdir(version)
        self.transformer = CLIPTextModel.from_pretrained(version, local_files_only=local_only)
        self.tokenizer = CLIPTokenizer.from_pretrained(version, local_files_only=local_only)
...
class FrozenCLIPEmbedder(nn.Module):
    def __init__(self, version="openai/clip-vit-large-patch14"):
        ...
        version = _resolve_hf_version(version)
        local_only = os.path.isdir(version)
        self.model = CLIPModel.from_pretrained(version, local_files_only=local_only)
