...
class BicubicDownSample(nn.Module):
    ...
    def forward(self, x, nhwc=False, clip_round=False, byte_output=False):
        ...
        filters1 = self.k1.to(device=x.device, dtype=torch.float32)
        filters2 = self.k2.to(device=x.device, dtype=torch.float32)
...
class FaceParser(nn.Module):
    ...
    def __init__(self, seg_ckpt, size = 1024, device=None):
    ...
    def load_downsampling(self):
        ...
        self.downsample = BicubicDownSample(factor=self.size // 512, cuda=use_cuda)
        self.downsample_256 = BicubicDownSample(factor=self.size // 256, cuda=use_cuda)
    ...
    def load_segmentation_network(self):
        ...
        self.seg.load_state_dict(torch.load(self.seg_ckpt, map_location=map_location))
...
def init_faceParsing_pretrained_model(faceParser_name, ckpt_path, config_path = ""):
    if faceParser_name == "default":
        ...
        parser = init_segmentor(config_path, ckpt_path, device=device)
